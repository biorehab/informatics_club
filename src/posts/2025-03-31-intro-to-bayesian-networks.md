...
<svg id="BN3Demo" width="400" height="300"></svg> -->
...
<div class="bn-container">
  ...
  <div class="bn-plot-area">
    ...
    <svg id="BN3Demo" width="300" height="250"></svg>
  </div>
</div>
...
<script>
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    const updatePlot = () => {
      const condition = document.getElementById("conditionCheckbox").checked;
      document.getElementById("bSlider").disabled = !condition;
      const bTarget = parseFloat(document.getElementById("bSlider").value);
      ...
      if (condition) {
        ...
        if (filtered.length > 2) {
          ...
          condCorrText.text(`X1~X3 | X2≈${bTarget.toFixed(1)}: ${condCorr.toFixed(2)}`);
        } else {
      ...
      corrText.text(`X1~X3: ${corr.toFixed(2)}`);
    };
    ...
    // Event listeners
    document.getElementById("conditionCheckbox").addEventListener("change", updatePlot);
    document.getElementById("bSlider").addEventListener("input", updatePlot);
  });
</script>
